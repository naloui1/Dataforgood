...
def creating_vusualisation(data: pd.DataFrame, nom_commune: str)-> go.Figure:
    df = data[data['Commune'] == nom_commune]
    ...
    # Bar Chart
    fig_bar = px.bar(df.sort_values(by=['count', 'categorie'], ascending=[False, True]), 
    ...
    fig_bar.update_traces(text=df.sort_values(by=['count', 'categorie'], ascending=[False, True])['count'], 
    ...
    commune_title = f"<b>Commune:</b> {df['Commune'].iloc[0]}"
    fig.add_annotation(
        text=commune_title,  
        x=0.5,  
        y=1.15,  
